...
impl MedicalScienceLaws {
    ...
    /// 免疫学规则
    pub fn immunology_rules(&self) -> Vec<&'static str> {
        vec![
            "先天免疫: 皮肤粘膜等非特异性防御的第一道防线",
            "适应性免疫: T细胞和B细胞介导的特异性免疫应答",
            "免疫记忆: 初次感染后再次遇到同种病原体反应更快更强",
            "疫苗原理: 用减毒灭活病原体刺激免疫系统产生保护性记忆",
            "自身免疫: 免疫系统错误攻击自身正常组织",
            "免疫耐受: 免疫系统对特定抗原不产生攻击性应答",
            "细胞因子: 免疫细胞间信号传递的蛋白质分子",
        ]
    }

    /// 药理学规则
    pub fn pharmacology_rules(&self) -> Vec<&'static str> {
        vec![
            "半衰期: 药物在体内血浆浓度降低一半所需时间",
            "治疗窗口: 药物既有效又安全的血药浓度范围",
            "首过效应: 口服药物经门静脉入肝代谢导致生物利用度降低",
            "竞争性拮抗: 拮抗剂与激动剂竞争同一受体结合位点",
            "量效关系: 药物剂量与产生的药理效应之间的关系",
            "耐药性: 长期使用某种药物后治疗效果减弱的现象",
        ]
    }
