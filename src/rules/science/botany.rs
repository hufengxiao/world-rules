...
impl BotanyLaws {
    ...
    /// 植物生理
    pub fn plant_physiology(&self) -> Vec<&'static str> {
        vec![
            "光合作用: 利用光能将CO₂和H₂O合成有机物释放O₂",
            "呼吸作用: 分解有机物释放能量供生命活动利用",
            "蒸腾作用: 水分从叶面蒸发驱动水分向上运输",
            "向光性: 植物向光源方向弯曲生长的特性",
            "春化作用: 低温处理促进植物开花的现象",
            "光周期现象: 日照长度影响植物开花的反应",
        ]
    ...
    /// 植物分类
    pub fn plant_taxonomy(&self) -> Vec<&'static str> {
        vec![
            "双名法: 属名+种加词的物种命名体系",
            "被子植物: 种子被果皮包被的开花植物",
            "裸子植物: 种子裸露不被果皮包被",
    ...
    pub fn plant_ecology(&self) -> Vec<&'static str> {
        vec![
            "光合作用效率: 植物将光能转化为化学能的效率",
            "水分利用效率: 植物每消耗单位水生产的干物质量",
            "植物群落: 特定区域内植物种群的集合",
            "植被垂直分带: 山地植被随海拔呈现带状分布",
            "植物与传粉者: 植物与传粉动物的协同进化",
