...
impl OpticsLaws {
    ...
    /// 几何光学
    pub fn geometric_optics(&self) -> Vec<&'static str> {
        vec![
            "反射定律: 入射角等于反射角且入射线反射线法线共面",
            "斯涅尔折射定律: n₁sinθ₁=n₂sinθ₂",
            "全内反射: 光从光密介质射向光疏介质时入射角大于临界角的完全反射",
            "薄透镜公式: 1/f=1/u+1/v",
            "费马原理: 光沿传播时间最短的路径传播",
            "棱镜色散: 不同波长光折射率不同导致白光分光",
        ]
    }

    /// 波动光学
    pub fn wave_optics(&self) -> Vec<&'static str> {
        vec![
            "杨氏双缝干涉: 两束相干光叠加形成等间距明暗条纹",
            "薄膜干涉: 薄膜上下表面反射光的干涉产生彩色条纹",
            "单缝衍射: 光通过狭缝时偏离直线传播展宽",
            "光栅方程: dsinθ=mλ决定衍射极大方向",
            "偏振: 光波电矢量振动方向的择优取向",
            "布儒斯特角: 反射光完全偏振时的入射角tanθ=n₂/n₁",
            "瑞利散射: 散射光强度与波长四次方成反比",
        ]
    }
    ...
    pub fn fiber_optics(&self) -> Vec<&'static str> {
        vec![
            "光纤结构: 纤芯包层和涂覆层组成",
            "数值孔径: 光纤接收光的能力NA=sinθmax",
            "多模光纤: 可以传输多个模式的光纤",
            "单模光纤: 只传输基模的光纤",
            "光纤损耗: 吸收散射和弯曲导致的光功率降低",
            "色散: 光脉冲在光纤中展宽的现象",
