...
impl ArchaeologyLaws {
    ...
    /// 测年方法
    pub fn dating_methods(&self) -> Vec<&'static str> {
        vec![
            "碳14测年: 利用¹⁴C衰变测定含碳有机物年龄上限约5万年",
            "热释光测年: 加热结晶矿物释放储存的能量测定年龄",
            "铀系测年: 利用铀系同位素衰变链测定碳酸盐年龄",
            "树轮年代学: 通过树木年轮宽度序列建立年代标尺",
            "古地磁测年: 利用岩石记录的地磁极性倒转序列定年",
            "钾氩测年: 利用⁴⁰K衰变为⁴⁰Ar测定火山岩年龄",
        ]
    }

    /// 考古方法
    pub fn archaeological_methods(&self) -> Vec<&'static str> {
        vec![
            "地层学: 通过堆积层序判断遗存的相对年代关系",
            "类型学: 按形态特征对遗物进行分类排比研究",
            "聚落考古: 通过聚落布局研究古代社会组织",
            "实验考古: 通过模拟实验复原古代技术过程",
            "民族考古学: 参照现代民族志类比解释考古遗存",
            "遥感考古: 利用航拍卫星影像发现地下遗存",
