...
impl EconomicsRules {
    ...
    /// 微观经济学
    pub fn microeconomics(&self) -> Vec<&'static str> {
        vec![
            "供需法则: 价格上升需求量减少供给量增加",
            "边际效用递减: 消费越多每单位商品带来的满足感越低",
            "弹性理论: 衡量一个变量对另一个变量变化的敏感程度",
            "帕累托最优: 无法在不损害他人的情况下改善某人境况",
            "科斯定理: 产权明确且交易成本为零时资源配置达到最优",
            "信息不对称: 交易双方拥有不等信息导致市场失灵",
            "道德风险: 一方因另一方无法观察而改变行为的风险",
        ]
    }

    /// 宏观经济学
    pub fn macroeconomics(&self) -> Vec<&'static str> {
        vec![
            "IS-LM模型: 产品市场与货币市场同时均衡的分析框架",
            "菲利普斯曲线: 失业率与通胀率之间的负相关关系",
            "奥肯定律: GDP每下降2%失业率约上升1%",
            "购买力平价: 汇率应使各国相同商品价格趋于一致",
            "乘数效应: 支出增加导致国民收入更大比例增加",
            "流动性陷阱: 利率极低时货币政策失去刺激效果",
            "拉弗曲线: 税率与税收收入之间存在倒U型关系",
        ]
    }
    ...
    pub fn financial_economics(&self) -> Vec<&'static str> {
        vec![
            "有效市场假说: 资产价格已反映所有可得信息",
            "资本资产定价模型: CAPM描述风险与预期收益关系",
            "期权定价: 布莱克-舒尔斯期权定价公式",
            "投资组合理论: 马科维茨的均值方差分析框架",
            "行为金融学: 心理因素对投资者行为和市场的影响",
            "随机游走: 股价变动不可预测的理论",
            "风险管理: VaR等风险度量和控制方法",
        ]
    }
