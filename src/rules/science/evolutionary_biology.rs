...
impl EvolutionaryBiologyLaws {
    ...
    /// 进化机制
    pub fn evolution_mechanisms(&self) -> Vec<&'static str> {
        vec![
            "自然选择: 适者生存不适者淘汰的进化驱动力",
            "遗传漂变: 小种群中基因频率随机变化",
            "基因流: 种群间个体迁移导致基因交流",
            "突变: DNA序列的随机变化是进化的原始材料",
            "性选择: 通过配偶选择推动特征进化",
            "协同进化: 两个物种相互影响共同进化",
        ]
    }
    ...
    /// 物种形成
    pub fn speciation(&self) -> Vec<&'static str> {
        vec![
            "异域物种形成: 地理隔离导致生殖隔离",
            "同域物种形成: 无地理隔离情况下形成新物种",
            "适应辐射: 一个祖先物种快速分化为多个物种",
            "趋同进化: 不同物种独立进化出相似特征",
            "退化进化: 适应寄生或穴居生活导致结构简化",
            "分子钟: DNA或蛋白质突变积累速率用于估算分歧时间",
        ]
    }
