...
impl EcologyLaws {
    ...
    /// 种群生态学
    pub fn population_ecology(&self) -> Vec<&'static str> {
        vec![
            "指数增长: 资源无限时种群呈J型增长",
            "逻辑增长: 资源有限时种群呈S型增长趋于K值",
            "种间竞争: 不同物种争夺相同资源",
            "捕食关系: 一个物种捕食另一个物种",
            "共生关系: 两个物种紧密生活在一起互利互惠",
            "寄生关系: 一方获利另一方受害",
        ]
    }

    /// 群落生态学
    pub fn community_ecology(&self) -> Vec<&'static str> {
        vec![
            "生态位: 物种在群落中的功能角色和位置",
            "竞争排斥: 生态位完全相同的两个物种不能共存",
            "边缘效应: 群落交错区物种多样性增加",
            "岛屿生物地理学: 岛屿物种数取决于面积和隔离程度",
            "营养级: 生物在食物链中的位置",
