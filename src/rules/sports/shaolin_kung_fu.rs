...
impl ShaolinKungFuRules {
    pub fn new() -> Self {
        Self {
            metadata: RuleMetadata::new("少林功夫规则", "少林功夫比赛基本规则")
                .with_origin("中国")
    ...
    pub fn competition_formats(&self) -> Vec<&'static str> {
        vec!["套路表演赛", "散打比赛", "器械比赛", "功夫表演", "团体表演"]
    }

    /// 基本套路
    pub fn routine_forms(&self) -> Vec<&'static str> {
        vec!["少林拳基础套路", "罗汉拳", "小洪拳", "大洪拳", "长拳套路"]
    }
    ...
    pub fn technique_requirements(&self) -> Vec<&'static str> {
        vec!["姿势正确", "动作连贯", "力度适中", "节奏把握", "精神饱满"]
    }

    /// 功夫特点
    pub fn characteristics(&self) -> Vec<&'static str> {
        vec!["刚柔相济", "内外兼修", "攻防结合", "禅武合一", "传统传承"]
    }

    /// 装备要求
    pub fn equipment(&self) -> Vec<&'static str> {
        vec!["武术服装", "武术器械", "比赛场地", "安全保护", "传统装束"]
    }
