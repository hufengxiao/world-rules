...
impl YogaCompetitionRules {
    pub fn new() -> Self {
        Self {
            metadata: RuleMetadata::new("瑜伽比赛规则", "瑜伽比赛基本规则")
                .with_origin("印度")
    ...
    pub fn posture_categories(&self) -> Vec<&'static str> {
        vec!["站立姿势", "坐姿", "卧姿", "倒立姿势", "平衡姿势"]
    }

    /// 评分标准
    pub fn scoring_criteria(&self) -> Vec<&'static str> {
        vec!["体式准确性", "柔韧度", "力量表现", "平衡能力", "呼吸配合"]
    }
    ...
    pub fn difficulty_levels(&self) -> Vec<&'static str> {
        vec!["初级体式", "中级体式", "高级体式", "专业体式", "艺术体式"]
    }

    /// 装备要求
    pub fn equipment(&self) -> Vec<&'static str> {
        vec!["瑜伽服", "瑜伽垫", "比赛服装规定", "无鞋比赛", "简约装饰"]
    }

    /// 场地要求
    pub fn field_requirements(&self) -> Vec<&'static str> {
        vec!["场地平整", "瑜伽垫铺设", "温度适宜", "光线良好", "音乐伴奏"]
    }
