...
- **Global scope**: `D:/Projects/world-rules/.hermes/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Projects/world-rules/.hermes/notes/`)
...
2. Glob `D:/Projects/world-rules/.hermes/notes/*.md` (if directory exists) — global notes
...
Global (D:/Projects/world-rules/.hermes/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Projects/world-rules/.hermes/notes/` — works in any directory
