//! Phase 19 新规则集成测试
//!
//! 测试 Phase 19-01 到 19-02 添加的棋类规则：
...
fn blind_go_challenge_levels() {
    ...
    assert!(explanation.contains("普通") || explanation.contains("完全"), "应说明挑战等级");
...
fn pair_go_competition() {
    ...
    assert!(explanation.contains("世界") || explanation.contains("国际"), "应说明国际赛事");
}
...
fn phase_19_go_variants_registered_in_module() {
    use world_rules::rules::games::{
        Go9x9Rules, Go13x13Rules, BlindGoRules, OneColorGoRules, PairGoRules,
    };
